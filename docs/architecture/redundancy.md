...
We strongly discourage connecting directly via IP addresses. Core nodes may be rotated, replaced, or relocated over time, and DNS is the only supported mechanism that guarantees stable, correct routing to the Core infrastructure.

## Club redundancy

Large clubs may choose to run multiple MQTT broker instances behind a load balancer or to split traffic by functional domains (HF vs VHF, contest vs everyday).
